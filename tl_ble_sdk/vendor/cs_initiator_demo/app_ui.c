...
u8 vc_detect_button(int read_key)
{
    ...
    if (btn_changed && read_key) {
        tlkapi_send_string_data(1, "[UI][PAIR] Button pressed", 0, 0);
        for (i = 0; i < MAX_BTN_SIZE; i++) {
...
#define CS_ALGO3_THD_STEP 0.01
            float thd = blc_cs_getAlgo3Thd();
            if (key0 == CR_VOL_UP) {
                thd += CS_ALGO3_THD_STEP;
            } else {
                thd -= CS_ALGO3_THD_STEP;
            }
            blc_cs_setAlgo3Thd(thd);
            app_parse_printf("cs thd:%f", thd);
        } else {
